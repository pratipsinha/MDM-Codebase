...
public class MassExportSingleFile implements ReportGenerateFunction
{
	...
	public void reportGenerate(ReportGenerateFunctionArguments arg0) 
	{
		...
		String outputFileName = System.getProperty(Constant.DOLLAR_TOP)+"/Output Files/Mass_Export/Core_Attribute_Export/Mass_Export_SingleFile_";
		...
		try
		{
			...
			query = "select item from catalog('Product Catalog')";
			...
			while(resultSet.next()) {
				...
				objLogger.logDebug("processing product ------------------->>"+ objItem.getPrimaryKey());
				for(int i = 1 ; i<=properties.size() ; i++)
				{
					String attribValue = "";
						...
						for(AttributeInstance associationLeafAttributeInstance : associationAtrribInstList)
						{
							...
							for(AttributeInstance associatedPartsInst : attrAssociatedPartsInstList)
							{
								...
								if(null!=associatedPartsInst.getValue())
								{
									...
									objLogger.logDebug(++count+"th attribValue for association is : "+associatedPartsInst.getValue());
								}
								...
								if(null!=linkedItem)
								{
									...
									{
										objLogger.logDebug("---->");
										objLogger.logDebug(linkedItem.getAttributeValue("Product Catalog Global Spec/SystemOfRecord"));
										if(null != linkedItem.getAttributeValue("Product Catalog Global Spec/SystemOfRecord"))
										associatedParts = attribValue+ " "+linkedItem.getAttributeValue("Product Catalog Global Spec/SystemOfRecord").toString();
										objLogger.logDebug("----------------->"+attribValue);
